...
const pmEsc=s=>(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot',"'":'&#039;'}[c]));
...
function pmLoadNotions(){
  ...
  return Promise.all(PM_FILES.map(f=>fetch(`./data/notions-${f}.json?v=5`).then(r=>r.json()))).then(all=>{
...
function pmSetMode(on){
  pmQuizMode=!!on;
  document.body.classList.toggle('pm-quiz-mode',pmQuizMode);
  pmEnsureQuizTab();
}
function pmEnsureQuizTab(){
  const nav=document.querySelector('.nav');
  if(!nav)return;
  nav.querySelectorAll('[data-v]').forEach(b=>{
    if(b.dataset.pmModeBound)return;
    b.dataset.pmModeBound='1';
    b.addEventListener('click',()=>pmSetMode(false),true);
  });
  let button=nav.querySelector('[data-pm-house-quiz]');
  if(!button){
    button=document.createElement('button');
    button.type='button';
    button.dataset.pmHouseQuiz='1';
    button.innerHTML='<span class="pm-tab-full">Maison / Révision</span><span class="pm-tab-short">Maison +</span>';
    button.title='Même maison, mais tu dois retrouver le mot et la définition';
    button.addEventListener('click',e=>{
      e.preventDefault();e.stopPropagation();
      const house=nav.querySelector('[data-v="house"]');
      if(house)house.click();
      pmSetMode(true);
    });
    nav.appendChild(button);
  }
  button.classList.toggle('pm-active',pmQuizMode);
}
...
function pmCorrectionHTML(n){
  return `<div class="pm-correction"><div class="pm-correction-title">Réponse de référence</div><div class="mnemonic-stage pm-original-mnemonic"><span class="visual">${pmEsc(n.emoji||'🧠')}</span><h3>${pmEsc(n.mnemonic||'Association mnémotechnique')}</h3><p class="muted"><b>Lien :</b> ${pmEsc(n.link||'')}</p></div><h2>${pmEsc(n.term)}</h2><div class="pm-ref-definition">${pmEsc(n.definition)}</div><button class="secondary pm-speak-answer">🔊 Lire le mot et la définition</button></div>`;
}
function pmAttachSpeak(el,n){
...
function pmOpenQuiz(n,button){
  ...
  el.innerHTML=`<div class="pm-quiz-modal"><div class="pm-quiz-head"><div><span class="pill">#${n.id} · ${already?'🟢 déjà validée':'à valider'}</span><h2>${icon} ${pmEsc(locus)}</h2><p>Retrouve le <b>mot</b> et sa <b>définition</b>. Ensuite tu retrouves exactement la même association visuelle que dans Maison.</p></div><button class="icon-btn pm-close">✕</button></div><div class="pm-quiz-form"><label>1. Quel est le mot / la notion ?<input class="pm-term-input" autocomplete="off" autocapitalize="sentences" placeholder="Écris le mot associé à cet objet…"></label><label>2. Donne sa définition<textarea class="pm-definition-input" placeholder="Écris la définition avec tes mots…"></textarea></label><button class="primary pm-check">✓ Vérifier mes deux réponses</button><div class="pm-feedback" aria-live="polite"></div><div class="pm-answer-zone"></div></div></div>`;
  ...
  el.querySelector('.pm-check').onclick=()=>{
    ...
    const answerZone=el.querySelector('.pm-answer-zone');
    answerZone.innerHTML=pmCorrectionHTML(n);
    pmAttachSpeak(el,n);
    if(termOk&&d.ok){
      ...
      feedback.innerHTML=`<b>Pas encore validé</b><span>${termLine}</span><span>${defLine}</span>${!d.ok&&d.missing.length?`<small>Quelques idées importantes à retrouver : ${d.missing.map(pmEsc).join(', ')}.</small>`:''}<small>L’association visuelle animée et la réponse apparaissent juste en dessous, comme dans Maison.</small>`;
    }
  };
}

document.addEventListener('click',async e=>{
  if(!pmQuizMode)return;
...
const pmObserver=new MutationObserver(()=>pmEnsureQuizTab());
const pmApp=document.getElementById('app');
if(pmApp)pmObserver.observe(pmApp,{childList:true,subtree:true});
window.addEventListener('DOMContentLoaded',()=>{pmEnsureQuizTab();pmRestoreRoom()});
setTimeout(()=>{pmEnsureQuizTab();pmRestoreRoom()},250);
